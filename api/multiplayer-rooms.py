    ...
    if supabase_url and supabase_key:
        try:
            ...
            for room in rooms:
                ...
                print(f"Supabase rooms save success: {response.status}")
            return True
        except urllib.error.HTTPError as e:
            error_body = e.read().decode() if e.fp else "No error body"
            print(f"Supabase rooms HTTP error {e.code}: {error_body}")
            return False
        except Exception as e:
            print(f"Supabase rooms save error: {type(e).__name__}: {e}")
            return False
    return False
